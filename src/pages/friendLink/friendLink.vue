<template>
  <div class="w-full h-full relativex py-8 flex flex-col items-center">
    <!--    友链列表-->
    <a :href="item.url"
       target="_blank"
      v-for="(item, index) in friendList"
      :key="index"
      class="flex w-5/6 h-24 bg-white rounded-lg py-2 px-4 overflow-hidden mb-4 animate__animated animate__bounceInUp"
    >
      <div class="w-20 h-20 mask mask-hexagon animate-pulse">
         <img :src="item.logo" />
      </div>
      <div class="flex flex-col justify-center ml-4">
        <div class="font-semibold">{{ item.name }}</div>
        <div class="opacity-40">{{ item.description }}</div>
      </div>
    </a>
    <!--    github偷来的小人-->
    <img
      class="w-96 h-96 absolute right-0 bottom-0  animate__animated animate__bounceInDown"
      src="http://124.71.57.28:8090/upload/2021/08/astro-mona-03a16b99728c4f6b97c584e0fd8b18f5.svg"
    />
  </div>
</template>
<script lang="ts" setup>
  import { reactive, toRefs } from 'vue';
  import { GetFriendLink } from './friendLink.ts';
  const state = reactive({
    friendList: [],
  });
  const { friendList } = { ...toRefs(state) };
  state.friendList = await GetFriendLink();
</script>
<style lang="scss" scoped>
  .mask-hexagon {
    -webkit-mask-image: url(data:image/svg+xml;base64,);
    mask-image: url(data:image/svg+xml;base64,);
  }
  .mask {
    -webkit-mask-size: contain;
    mask-size: contain;
    -webkit-mask-repeat: no-repeat;
    mask-repeat: no-repeat;
    -webkit-mask-position: center;
    mask-position: center;
  }
</style>
