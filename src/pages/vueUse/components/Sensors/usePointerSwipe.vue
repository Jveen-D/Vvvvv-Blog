...
const info = reactive(usePointerSwipe(target, {
    onSwipe() {
        if (containerWidth.value) {
            if (info.distanceX < 0) {
                const distance = Math.abs(info.distanceX)
    ...
    onSwipeEnd() {
        if (info.distanceX < 0 && containerWidth.value && (Math.abs(info.distanceX) / containerWidth.value) >= 0.5) {
            left.value = '100%'
            opacity.value = 0
        }
        else {
